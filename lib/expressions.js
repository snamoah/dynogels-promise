  ...
  const result = _.reduce(
    ...
    (result, value, key) => {
      ...
      if (_.isNull(value) || (_.isString(value) && _.isEmpty(value))) {
        ...
        result.values[valueKey] = serializer.serializeAttribute(
          value.$add,
          datatypes[key]
        )
        result.attributeNames[nameKey] = key
...
internals.isFunctionOperator = operator =>
  _.includes(
    [
      'attribute_exists',
...
exports.buildFilterExpression = (
  ...
  const v2ValueName = internals.uniqAttributeValueName(
    key,
    [v1ValueName].concat(existingValueNames)
  )
