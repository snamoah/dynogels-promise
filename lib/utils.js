  ...
  const merged = _.reduce(
    ...
    (memo, resp) => {
      ...
      if (resp.ConsumedCapacity) {
        memo.ConsumedCapacity.CapacityUnits +=
...
utils.omitPrimaryKeys = (schema, params) =>
  _.omit(params, schema.hashKey, schema.rangeKey)

utils.strToBin = value => {
  if (typeof value !== 'string') {
    const StrConversionError =
      'Need to pass in string primitive to be converted to binary.'
