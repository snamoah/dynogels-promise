...
internals.deserializeAttribute = value => {
  if (
    _.isObject(value) &&
    _.isFunction(value.detectType) &&
    _.isArray(value.values)
  ) {
...
internals.serializeAttribute = serializer.serializeAttribute = (
  value,
  type
) => {
  if (!type) {
...
serializer.buildKey = (hashKey, rangeKey, schema) => {
  ...
  if (_.isPlainObject(hashKey)) {
    ...
    if (
      schema.rangeKey &&
      !_.isNull(hashKey[schema.rangeKey]) &&
      !_.isUndefined(hashKey[schema.rangeKey])
    ) {
