...
const promisifiedCallback = (callback, resolve, reject) => (err, data) => {
...
function Table (name, schema, serializer, docClient, logger) {
...
Table.prototype.initItem = function (attrs) {
  ...
  if (self.itemFactory) {
    const Factory = self.itemFactory
...
Table.prototype.create = function (item, options, callback) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (_.isArray(item)) {
      async.map(
        item,
        (data, callback) => internals.createItem(self, data, options, callback),
        promisifiedCallback(callback, resolve, reject)
      )
    } else {
...
internals.createItem = (table, item, options, callback) => {
  ...
  const start = callback => {
    ...
    const paramName = _.isString(self.schema.createdAt)
      ? self.schema.createdAt
      : 'createdAt'

    if (
      self.schema.timestamps &&
      self.schema.createdAt !== false &&
      !_.has(data, paramName)
    ) {
  ...
  internals.callBeforeHooks(self, 'create', start, (err, data) => {
    ...
    if (options.overwrite === false) {
      const expected = _.chain([self.schema.hashKey, self.schema.rangeKey])
        .compact()
        .reduce((result, key) => {
    ...
    self.sendRequest('put', params, (err, data) => {
      if (err) {
        return callback(err)
      }
...
internals.validateItemFragment = (item, schema) => {
  ...
  const setOperationValues = _.pickBy(
    item,
    i => _.isPlainObject(i) && (i.$add || i.$del)
  )
  ...
  const removalValidation = schema.validate({}, { abortEarly: false })

  if (removalValidation.error) {
    const errors = _.pickBy(
      removalValidation.error.details,
      e =>
        _.isEqual(e.type, 'any.required') &&
  ...
  const updateValidation = schema.validate(updateAttributes, {
    abortEarly: false
  })

  if (updateValidation.error) {
    const errors = _.omitBy(updateValidation.error.details, e =>
      _.isEqual(e.type, 'any.required')
    )
    if (!_.isEmpty(errors)) {
      error.update = errors
  ...
  return new Promise((resolve, reject) => {
    ...
    if (schemaValidation.error) {
      const error = _.assign(
        new Error(
          `Schema validation error while updating item in table ${self.tableName()}: ${JSON.stringify(
    ...
    const start = callback => {
      const paramName = _.isString(self.schema.updatedAt)
        ? self.schema.updatedAt
        : 'updatedAt'

      if (
        self.schema.timestamps &&
        self.schema.updatedAt !== false &&
        !_.has(item, paramName)
      ) {
    ...
    internals.callBeforeHooks(self, 'update', start, (err, data) => {
      ...
      const unprocessedOptions = _.omit(options, [
        'UpdateExpression',
      ...
      params = _.chain({})
        .merge(params, unprocessedOptions)
        .omitBy(_.isEmpty)
        .value()
      ...
      self.sendRequest('update', params, (err, data) => {
        ...
        let result = null
        if (data.Attributes) {
          result = self.initItem(
            self.serializer.deserializeItem(data.Attributes)
          )
        }
...
internals.addConditionExpression = (params, expectedConditions) => {
  _.each(expectedConditions, (val, key) => {
    ...
    const condition = expressions.buildFilterExpression(
      key,
      operator,
      existingValueKeys,
      expectedValue,
    ...
    params.ExpressionAttributeNames = _.merge(
      {},
      condition.attributeNames,
      params.ExpressionAttributeNames
    )
    params.ExpressionAttributeValues = _.merge(
      {},
...
internals.deserializeItems = (table, callback) => (err, data) => {
  ...
  const result = {}
  if (data.Items) {
    result.Items = _.map(data.Items, i =>
      table.initItem(table.serializer.deserializeItem(i))
    )
...
Table.prototype.runQuery = function (params, callback) {
  ...
  return new Promise((resolve, reject) => {
    ...
    self.sendRequest(
      'query',
      params,
      internals.deserializeItems(self, promisifiedCallback)
    )
  })
}
...
Table.prototype.runBatchGetItems = function (params, callback) {
  ...
  return new Promise((resolve, reject) => {
    ...
    self.sendRequest('batchGet', params, promisifiedCallback)
  })
}
...
Table.prototype.dynamoCreateTableParams = function (options) {
  ...
  attributeDefinitions.push(
    internals.attributeDefinition(self.schema, self.schema.hashKey)
  )

  if (self.schema.rangeKey) {
    attributeDefinitions.push(
      internals.attributeDefinition(self.schema, self.schema.rangeKey)
    )
  }
  ...
  _.forEach(self.schema.secondaryIndexes, params => {
    attributeDefinitions.push(
      internals.attributeDefinition(self.schema, params.rangeKey)
    )
  ...
  _.forEach(self.schema.globalIndexes, (params, indexName) => {
    if (!_.find(attributeDefinitions, { AttributeName: params.hashKey })) {
      attributeDefinitions.push(
        internals.attributeDefinition(self.schema, params.hashKey)
      )
    }

    if (
      params.rangeKey &&
      !_.find(attributeDefinitions, { AttributeName: params.rangeKey })
    ) {
      attributeDefinitions.push(
        internals.attributeDefinition(self.schema, params.rangeKey)
      )
  ...
  const keySchema = internals.keySchema(
    self.schema.hashKey,
    self.schema.rangeKey
  )
  ...
  if (
    Object.prototype.hasOwnProperty.call(options, 'streamSpecification') &&
    typeof options.streamSpecification === 'object'
  ) {
    ...
    if (params.StreamSpecification.StreamEnabled) {
      params.StreamSpecification.StreamViewType =
        options.streamSpecification.streamViewType || 'NEW_AND_OLD_IMAGES'
    }
  }
  return params
}
...
Table.prototype.createTable = function (options, callback) {
  ...
  return new Promise((resolve, reject) => {
    ...
    self.sendRequest('createTable', params, promisifiedCallback)
  })
}
...
Table.prototype.describeTable = function (callback) {
  ...
  return new Promise((resolve, reject) => {
    ...
    self.sendRequest('describeTable', params, promisifiedCallback)
  })
}
...
Table.prototype.deleteTable = function (callback) {
  ...
  return new Promise((resolve, reject) => {
    ...
    self.sendRequest('deleteTable', params, promisifiedCallback)
  })
}
...
internals.syncIndexes = (table, callback) => {
  ...
  table.describeTable((err, data) => {
    ...
    async.mapLimit(
      missing,
      maxIndexCreationsAtaTime,
      (params, callback) => {
        const attributeDefinitions = []

        if (!_.find(attributeDefinitions, { AttributeName: params.hashKey })) {
          attributeDefinitions.push(
            internals.attributeDefinition(table.schema, params.hashKey)
          )
        }
        ...
        if (
          ...
          attributeDefinitions.push(
            internals.attributeDefinition(table.schema, params.rangeKey)
          )
        }

        const currentWriteThroughput =
