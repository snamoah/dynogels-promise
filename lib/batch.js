...
internals.serializeKeys = (keys, table, serializer) =>
  keys.map(key => serializer.buildKey(key, null, table.schema))
...
internals.mergeResponses = (tableName, responses) => {
  ...
  return responses.reduce((memo, resp) => {
    if (resp.Responses && resp.Responses[tableName]) {
      memo.Responses[tableName] = memo.Responses[tableName].concat(
...
internals.initialBatchGetItems = (
  keys,
  table,
  serializer,
  options,
  ...
  const request = internals.buildInitialGetItemsRequest(
    table.tableName(),
    serializedKeys,
    options
  )
  ...
  internals.paginatedRequest(request, table, (err, data) => {
    ...
    const items = _.map(dynamoItems, i =>
      table.initItem(serializer.deserializeItem(i))
    )
  ...
  return Promise((resolve, reject) => {
    ...
    async.map(
      ...
      (err, results) => {
        ...
        return promisifiedCallback(null, _.flatten(results))
      }
    )
  })
}
